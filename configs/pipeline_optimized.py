...
class OptimizedPipeline:
    ...
    async def run(self) -> bool:
        ...
        try:
            ...
            # Фикс регрессии свёрнутых логов: tqdm в stderr невидим в GitHub Actions,
            # заменяем на периодические logger.info с прогрессом каждые 5%.
            total_raw = max(1, len(raw_configs))
            log_every = max(50, total_raw // 20)
            logger.info(f"▶ Парсинг конфигов: всего={total_raw}, шаг_лога={log_every}")
            for i, cfg in enumerate(raw_configs):
                ...
                if (i + 1) % log_every == 0:
                    pct = round(100.0 * (i + 1) / total_raw, 1)
                    logger.info(f"  … парсинг {i + 1}/{total_raw} ({pct}%), valid={len(valid_configs)}")
            ...
            scored_configs = []
            total_to_score = max(1, len(filtered_by_sni))
            score_log_every = max(50, total_to_score // 20)
            logger.info(f"▶ Скоринг конфигов: всего={total_to_score}, шаг_лога={score_log_every}")
            for idx, cfg in enumerate(filtered_by_sni):
                ...
                if (idx + 1) % score_log_every == 0:
                    pct = round(100.0 * (idx + 1) / total_to_score, 1)
                    avg = round(sum(s['score'] for s in scored_configs) / max(1, len(scored_configs)), 2)
                    logger.info(f"  … скopинг {idx + 1}/{total_to_score} ({pct}%), scored={len(scored_configs)}, avg={avg}")
