...
class IntelligentProbe:
    ...
    async def probe(self, config: str, parsed: dict = None) -> Dict:
        if parsed is None:
        ...
        for attempt in range(1, self.max_attempts + 1):
            probe_result = await self._single_probe(
                ...
                parsed, attempt, self.max_attempts
            )
    ...
    async def _single_probe(self, protocol, transport, host, port, sni, host_header, path,
                            parsed, attempt, total_attempts) -> Dict:
        ...
        try:
            # 1. TCP/TLS рукопожатие
            tls_start = time.time()
            if protocol in ('vless', 'vmess', 'trojan', 'ss') or transport in ('tcp', 'ws', 'grpc', 'xhttp', 'httpupgrade'):
                ...
                result['tls_handshake'] = (time.time() - tls_start) * 1000
                # Закрываем соединение (для простых протоколов)
                writer.close()
                await writer.wait_closed()
            else:
                # Если не удалось установить соединение – ошибка
                result['error'] = 'unsupported_transport'
                return result

            # 2. Протокол-специфичный зонд
            session = await SessionPool().get_session()
            scheme = 'https' if parsed.get('security') in ('tls', 'reality') else 'http'
            url = f"{scheme}://{host}:{port}{path}"

            headers = {'Host': host_header} if host_header else {}
            if protocol == 'vmess':
                # Дополнительная проверка alterId (если есть)
                ...
                if method and password:
                    # Можно отправить зашифрованный эхо-запрос (упрощённо)
                    pass

            # HTTP-запрос
            http_start = time.time()
            async with session.get(url, headers=headers, timeout=ClientTimeout(total=self.timeout)) as resp:
