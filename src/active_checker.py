"""
Активная проверка прокси-конфигураций.

Исправления и оптимизации:
- короткий TTL для негативного кеша и длинный для успешного;
- реальное переиспользование keep-alive через SessionPool;
- HTTP/WS-friendly проба с GET+Range на реальный path конфига;
- дедупликация batch-проверок и переиспользование одного SNIProbe.
"""
...
from user_settings import TCP_TIMEOUT, HTTP_TIMEOUT, MAX_LATENCY_MS, ACTIVE_CHECKER_WORKERS, PER_HOST_LIMIT
...
class CachedActiveChecker:
    ...
    async def _get_session(self) -> aiohttp.ClientSession:
        return await SessionPool().get_session(
